    ...
    <label className="flex items-center gap-3 cursor-pointer">
      <div className="relative">
        <input
          ...
          aria-label={label}
        />
        <div className={`w-10 h-5 rounded-full transition-colors duration-200 ${checked ? 'bg-indigo-600' : 'bg-gray-200'}`} />
...
function ProgressBar({ step }: { step: number }) {
  const steps = ['Event', 'Services', 'Details', 'Review', 'Options']
  return (
...
const SERVICE_META: Record<string, { Icon: HeroIcon; label: string; sub?: string }> = {
  ...
  AGENT_CHOOSES: { Icon: UserGroupIcon,      label: 'Agent Chooses', sub: 'Let the booking agent select services' },
}
...
export function TravelRequestForm({ hasDriversLicense }: { hasDriversLicense: boolean }) {
  ...
  function toggleService(s: string) {
    if (s === 'AGENT_CHOOSES') {
      setServices(['AGENT_CHOOSES'])
    } else {
      setServices(prev => {
        const without = prev.filter(x => x !== 'AGENT_CHOOSES')
  ...
  async function fetchAiOptions(requestId: string) {
    ...
    if (!res.ok) { setAiError('Could not load options'); setAiLoading(false); return }
      ...
      <div className="bg-white rounded-2xl shadow-sm p-6 sm:p-8 space-y-6">
        ...
        {step === 2 && (
          <div>
            ...
            {carLicenseWarning && (
              <div className="col-span-2 flex items-start gap-2 rounded-xl bg-amber-50 border border-amber-200 px-4 py-3">
                <svg className="w-4 h-4 text-amber-500 mt-0.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}>
                  <path strokeLinecap="round" strokeLinejoin="round" d="M12 9v3.75m-9.303 3.376c-.866 1.5.217 3.374 1.948 3.374h14.71c1.73 0 2.813-1.874 1.948-3.374L13.949 3.378c-.866-1.5-3.032-1.5-3.898 0L2.697 16.126zM12 15.75h.007v.008H12v-.008z" />
                </svg>
                <div className="text-sm text-amber-800">
                  To book a car rental you need to add your driver&apos;s license.{' '}
                  <a href="/employee/profile" className="font-semibold underline hover:text-amber-900">
                    Go to my profile →
                  </a>
                </div>
              </div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {(['FLIGHT', 'HOTEL', 'CAR_RENTAL', 'TAXI'] as const).map(svc => {
                ...
                const locked  = svc === 'CAR_RENTAL' && !hasDriversLicense
                return (
                  <button
                    key={svc}
                    type="button"
                    onClick={() => {
                      if (locked) { setCarLicenseWarning(true); return }
                      setCarLicenseWarning(false)
                      toggleService(svc)
                    }}
                    className={`relative rounded-2xl border-2 p-5 flex flex-col items-center gap-2 transition-all ${
                      locked
            ...
            {services.includes('HOTEL') && (
              <section className="space-y-4">
                ...
                <Field label="Number of rooms">
                  <input type="number" min="1" max="20" title="Number of rooms" placeholder="1" value={hotel.rooms} onChange={e => setHotel(h => ({ ...h, rooms: e.target.value }))} className={inputCls} />
                </Field>
          ...
          <div className="space-y-5">
            ...
            {aiLoading && (
              <div className="flex flex-col items-center py-12 gap-3 text-gray-400">
                ...
                <p className="text-sm">Finding available options…</p>
              </div>
            )}
            ...
            {aiError && (
              <div className="rounded-xl bg-red-50 border border-red-200 p-4 flex items-center justify-between gap-3">
                ...
                <button
                  type="button"
                  onClick={() => createdRequestId && fetchAiOptions(createdRequestId)}
            ...
            {aiOptions && SERVICE_MAPPING
              .filter(({ key, type }) => services.includes(type) && (aiOptions[key]?.length ?? 0) > 0)
                  ...
                  <div className="space-y-2">
                    {aiOptions[key].map((opt, i) => {
                      ...
                      return (
                        ...
                        >
                          <div className="min-w-0">
                            <p className="font-medium text-gray-900 text-sm">{opt.vendor}</p>
                            <p className="text-xs text-gray-500 truncate">{opt.description}</p>
                          </div>
                          <div className="flex items-center gap-3 shrink-0">
                            <span className="font-semibold text-gray-900">${opt.priceUsd}</span>
        ...
        {step === 5 ? (
          <div className="flex gap-3 pt-2 justify-end">
            ...
            <button
              ...
              disabled={confirmingOptions || aiLoading}
            ...
            >
              {confirmingOptions ? 'Saving…' : 'Confirm selections →'}
            </button>
          </div>
        ) : (
