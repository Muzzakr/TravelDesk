          ...
          <div className="space-y-8">
            ...
            {services.includes('HOTEL') && (
              <section className="space-y-4">
                ...
                <Combobox<string> label="City" required
                  selectedItem={hotel.city || null}
                  onSelectedItemChange={v => setHotel(h => ({ ...h, city: v ?? '' }))}
                  onInputValueChange={v => setHotel(h => ({ ...h, city: v }))}
                  items={HOTEL_CITIES} itemToString={s => s ?? ''} filterItems={locationFilter}
                  renderItem={s => <span>{s}</span>} placeholder="Search city…" emptyMessage="No cities found" />
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
