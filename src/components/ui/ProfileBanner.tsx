...
export function ProfileBanner({ missingFields, userId }: Props) {
  const [dismissed, setDismissed] = useState(true)

  useEffect(() => {
    // Show the banner each session (sessionStorage resets on tab/window close)
    ...
    <div className="mx-4 mt-4 md:mx-8 md:mt-6 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3.5 shadow-sm">
      <div className="flex items-start gap-3">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-sm font-semibold text-amber-900">Your travel profile is incomplete</p>
